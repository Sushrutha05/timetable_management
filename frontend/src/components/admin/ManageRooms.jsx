...
const ManageRooms = () => {
  ...
  const [editData, setEditData] = useState({ roomNumber: '', type: 'CLASSROOM', capacity: '' });
  const [saving, setSaving] = useState(false);
  const [deletingId, setDeletingId] = useState(null);

  const { sortedData, handleSort, sortIcon } = useSortableTable(rooms, 'roomNumber');
  ...
  const loadRooms = async (forceRefresh = false) => {
    ...
    try {
      ...
      const data = await cache.getOrFetch(CACHE_KEY, () => roomAPI.getAll());
      setRooms(data);
  ...
  const handleSubmit = async (e) => {
    ...
    setMessage({ type: '', text: '' });
    try {
      await roomAPI.create({ ...formData, capacity: parseInt(formData.capacity) });
  ...
  const handleBulkUpload = async (e) => {
    ...
    try {
      ...
      const result = await roomAPI.bulkUpload(fd);
      const count = Array.isArray(result) ? result.length : 0;
      setBulkMessage({ type: 'success', text: `Successfully imported ${count} room${count === 1 ? '' : 's'}.` });
      setBulkFile(null);
  ...
  const startEdit = (room) => { setEditingId(room.id); setEditData({ roomNumber: room.roomNumber || '', type: room.type || 'CLASSROOM', capacity: String(room.capacity ?? '') }); setMessage({ type: '', text: '' }); };
  const cancelEdit = () => { setEditingId(null); setEditData({ roomNumber: '', type: 'CLASSROOM', capacity: '' }); };
  ...
  const handleEditSave = async () => {
    ...
    setSaving(true);
    try {
      await roomAPI.update(editingId, { ...editData, capacity: parseInt(editData.capacity) });
      setSaving(false);
      setEditingId(null);
    ...
    <div>
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md mb-6">
          ...
          <form onSubmit={handleSubmit}>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Room Number <span className="text-red-500">*</span></label>
                <input type="text" required value={formData.roomNumber} onChange={(e) => setFormData({ ...formData, roomNumber: e.target.value })} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md dark:bg-gray-700 dark:text-white" />
              </div>
              <div>
                ...
                <select required value={formData.type} onChange={(e) => setFormData({ ...formData, type: e.target.value })} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md dark:bg-gray-700 dark:text-white">
                  <option value="CLASSROOM">Classroom</option>
                  <option value="LAB">Lab</option>
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Capacity <span className="text-red-500">*</span></label>
                <input type="number" required min="1" value={formData.capacity} onChange={(e) => setFormData({ ...formData, capacity: e.target.value })} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md dark:bg-gray-700 dark:text-white" />
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300 mb-4">Upload a CSV file with the required headers to add multiple rooms at once.</p>
          {bulkMessage.text && (
          ...
          <form onSubmit={handleBulkUpload}>
            <div className="flex flex-col gap-4">
              ...
              <div className="flex items-center justify-between text-sm text-gray-600 dark:text-gray-300 gap-2 flex-wrap">
                <a href={roomTemplate} download="room_template.csv" className="text-blue-600 dark:text-blue-400 hover:underline">Download sample CSV template</a>
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleEditSave(); }}>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" required value={editData.roomNumber} onChange={(e) => handleEditChange('roomNumber', e.target.value)} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md dark:bg-gray-700 dark:text-white" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Type</label>
                <select required value={editData.type} onChange={(e) => handleEditChange('type', e.target.value)} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md dark:bg-gray-700 dark:text-white">
                  <option value="CLASSROOM">Classroom</option>
                  <option value="LAB">Lab</option>
                </select>
              </div>
              <div>
                ...
                <input type="number" required min="1" value={editData.capacity} onChange={(e) => handleEditChange('capacity', e.target.value)} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md dark:bg-gray-700 dark:text-white" />
              </div>
            </div>
            ...
            <table className="w-full">
              <thead className="bg-gray-50 dark:bg-gray-700">
                <tr>
                  ...
                  <th className={thClass} onClick={() => handleSort('roomNumber')}>Room Number{sortIcon('roomNumber')}</th>
                  <th className={thClass} onClick={() => handleSort('type')}>Type{sortIcon('type')}</th>
                  <th className={thClass} onClick={() => handleSort('capacity')}>Capacity{sortIcon('capacity')}</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {sortedData.length === 0 ? (
                  <tr><td colSpan="5" className="px-4 py-4 text-center text-gray-500 dark:text-gray-400">No rooms found</td></tr>
                ) : (
                  sortedData.map((room, idx) => (
                    <tr key={room.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      <td className="px-4 py-3 text-sm">
                        <div className="flex gap-2">
                          <button onClick={() => startEdit(room)} className="px-3 py-1 bg-yellow-500 hover:bg-yellow-600 text-white rounded-md">Edit</button>
                          <button onClick={() => handleDelete(room.id)} disabled={deletingId === room.id} className="px-3 py-1 bg-red-600 hover:bg-red-700 text-white rounded-md disabled:opacity-60">
